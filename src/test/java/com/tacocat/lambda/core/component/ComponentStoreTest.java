...
public class ComponentStoreTest {
	...
	public void testRegisterEntity() {
		...
		Component[] expectedPositions = {entity.get(MockBehavior.p1), entity.get(MockBehavior.p2)};
		Assert.assertArrayEquals("Components can be looked up from store by type",
			expectedPositions,
			store.find(Position.class).toArray()
		);
